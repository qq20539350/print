...
//    [printer appendFooter:nil];
    [printer appendImage:[UIImage imageNamed:@"yellowmen"] alignment:HLTextAlignmentCenter maxWidth:341];
...
- (void)rightAction
{
    ...
    [[SEPrinterManager sharedInstance] sendPrintData:mainData completion:^(CBPeripheral *connectPerpheral, BOOL completion, NSString *error) {
        NSLog(@"写入结：%d---:%@",completion,error);
        NSLog(@"完成打印 -----");
    }];
//
//    [[SEPrinterManager sharedInstance] printWithResult:^(CBPeripheral *connectPerpheral, BOOL completion, NSString *error) {
//        NSLog(@"完成打印 -----");
//    }];
    //方式二：
