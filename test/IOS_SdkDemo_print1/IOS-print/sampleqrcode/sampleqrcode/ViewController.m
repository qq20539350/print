...
- (void)handleConnect:(id)sender
{
    ...
    dispatch_async(myApp.myQueue, ^{
        ...
        if([myApp.myBle Open:peripheral])
        {
            ...
            // 连接成功之后，打印一张测试页。
            [myApp.myPos POS_S_SetQRCode:"ABCDEFG" nUnitWidth:6 nVersion:0 nErrorCorrectionLevel:4];
            [myApp.myPos POS_S_SetQRCode:"电子" nUnitWidth:6 nVersion:0 nErrorCorrectionLevel:4];
        }
        else
        {
            dispatch_async(dispatch_get_main_queue(), ^{
