...
- (void)handleConnect:(id)sender
{
    ...
    dispatch_async(myApp.myQueue, ^{
        ...
        if([myApp.myBle Open:peripheral])
        {
            ...
            //[myApp.myBle Write:<#(Byte *)#> offset:<#(int)#> count:<#(int)#>];
            [myApp.myPos POS_PrintText:"测试页\n" x:-2 nWidthTimes:1 nHeightTimes:1 nFontType:0 nFontStyle:0x1B | 0x40];
            // 连接成功之后，打印一张测试页。
...
//            [myApp.myLabel DrawQRCode:12 starty:200 version:0 ecc:4 unitwidth:4 rotate:0 str:"Caysn"];
            ...
            [myApp.myPos POS_PrintPicture:img x:-2 nWidth:200 nHeight:200 nBinaryAlgorithm:1 nCompressMethod:0];
